...
The current release is `0.15.0`. It is shown in the application header, stored in the image's OCI version label, and published as the GHCR tag `0.15.0`. `package.json` is the release-version source used by the frontend and GitHub Actions. Portainer follows the rolling `main` image tag from `compose.yaml`.
