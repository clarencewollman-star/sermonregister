...
- People retain the internal `active` field for future administration. Songs, Texts, and Vorraden remain available and do not expose an Active/Inactive status. An unused Vorrade may be deleted; a Vorrade referenced by any service is protected from deletion.
...
Vorraden are reusable Lehr-only records. A Lehr may omit a Vorrade, and Vorrade By remains optional even when a Vorrade is selected. The Vorraden view shows Vorrade, the first line of Notes, Times Used, Last Used, and attachment count. It searches Title and Notes; sorts by Vorrade, Times Used, or Last Used; and uses a compact iPhone list. The full-screen editor saves an existing record when focus leaves a field and requires one explicit Save Vorrade for a new record. Its read-only Usage History shows each Lehr date and the Text used with it. Renaming, relinking, creating a separate record, and deliberate duplicate merging follow the identity-safe Text behavior. Merging preserves all linked services and attachments and asks which Notes to keep only when both records contain different Notes. Vorraden do not use tags, an inactive state, or a separate print report. The service selector is searchable and alphabetical, permits free typing, and creates a new record only when the service is saved.
...
Service-entry type-ahead lists place the six most recently used unique Songs, Texts, and People first. Remaining choices follow alphabetically, and free typing still creates new reusable records through the normal save flow. Vorraden remain purely alphabetical rather than receiving a recent-use section.
...
The iPhone interface should use a responsive list and focused edit form instead of squeezing all columns into a grid. Both interfaces use the same records, validation, and server operations. Below the existing 768-pixel breakpoint, the Register uses a compact two-line toolbar: Search and Add share the first line, and Service Type, Year, and Tags share the second; the service count and page subtitle are hidden. The fixed bottom navigation uses 44-pixel touch targets and reserves both the detected iPhone safe area and additional separation below the buttons so the home-indicator swipe does not collide with navigation taps.

The Texts page and main Register share the same selected sermon-tag filters. Multiple selected tags use match-all behavior and combine with the normal search query. The Texts search also recognizes one case-insensitive `before:` command without adding another search control. `before:2024` means strictly before January 1, 2024, while `before:2024-06-15` uses that exact exclusive cutoff. A matching Text must have at least one counted use and its calculated Last Used date must be earlier than the cutoff, so never-used Texts and Texts used on or after the cutoff are excluded. Ordinary search words and tag filters may be combined with the command. Invalid or repeated commands show a clear validation message and no results. The tag menu shows the six most-used tags first, followed by all used tags alphabetically, with each count representing unique Text records rather than service occurrences. Desktop filtering applies immediately. On iPhone, a compact Tags button opens a full-width selection panel and applies the draft selection deliberately. Selected filters remain while navigating between Register and Texts or opening editors, but a fresh visit starts unfiltered. Desktop Register rows show a linked Text's tag badges beneath its Description; compact iPhone service rows omit badges to preserve their density.

### Selected Text print report

The Texts page has a temporary selection mode so normal browsing remains uncluttered. In selection mode, tapping a row selects it instead of opening its editor. Select All Visible adds every Text matching the current search and tag filters; removing visible selections does not disturb selected Texts hidden by another filter. The selection count remains visible, and selected Texts retain the page's current sort order in the report. Selections and the editable report title last only for the current browser session and are cleared deliberately when selection mode is left.

Preview Report opens a read-only, ink-friendly U.S. Letter portrait preview before invoking the device's standard print dialog. The report is a clean two-column reading list rather than a form or spreadsheet: each numbered Text groups its available Description, Scripture Reference, comma-separated Tags, first line of Notes, Times Used, and Last Used with typography and whitespace instead of column headings, empty cells, boxes, or dividers. Blank values are omitted. Text and Description use at most two lines; Scripture Reference uses up to four lines; Tags and Notes use one line. Attachments, Songs For This Sermon, and full usage history are excluded. Each page reads down the left column and then down the right. Pagination measures the records as rendered and fits as many as the printable area allows, with no arbitrary record-count limit and without splitting a record. Each page shows a calculated Page X Of Y footer. The full editable report title, preparation date, and selected count appear on the first page only.

### Annual Register print report

The dedicated Reports navigation page contains an Annual Lehr Register card. Its year selector defaults to the newest available year and also offers All Years. The report ignores Register search, type, tag, and screen-year filters so an unnoticed screen filter cannot produce an incomplete archival report. Preview Register opens a read-only preview with Back To Reports, an immediately applied report-year selector, an Include Gebets checkbox that is selected by default, and Print. Clearing Include Gebets immediately recalculates the preview, yearly service totals, and page counts to print only Lehr services.

The annual report uses U.S. Letter portrait pages with equal 0.85-inch left and right margins so either single- or double-sided pages have comfortable three-ring-punch clearance. Services print oldest to newest within each year. On the same date, Lehr services print before Gebets, while records of the same type retain their entry order. All Years prints the oldest year first, and every year begins on the next available printed page without inserting a blank sheet. Each page repeats the Lehr Register title, year, that year's service count, and Continued on later pages. Page numbering restarts for each year, and every footer includes the print date.

Each service is a compact row with a thin horizontal divider and no column-heading row. Lehr rows retain the 9-point base type and are distinguished with a narrow dark left rule and stronger outlined Lehr label instead of an ink-heavy background fill. Gebet rows use a slightly smaller 8.25-point base type so the Lehr services remain visually dominant while Gebets stay readable. The primary line contains the full weekday/date including year, an ink-friendly outlined Lehr or Gebet label, the Text, up to one line of Text Description, and Text By. An optional flexible detail line contains only nonblank Song, Song By, Vorrade, Vorrade By, and complete Notes values with compact labels; it disappears when every secondary value is blank. Tags, Scripture Reference, Lehr progress and Status, attachments, and the unsaved inline-entry row are excluded. Pagination measures rendered row heights, fits as many complete rows as the page permits, and avoids splitting normal rows.

Each iPhone service is a flat AdminLTE list-group row inside the outlined Register card. The row shows `Sun · Aug 16, 2026`, a title-case Lehr or Gebet badge, and the same saved Status badge used by the desktop Register when that record has a Status. It also shows a one-line linked Text name, up to two lines of linked Text Description, and a final one-line `CCW · ♪ Song Title` summary. Long preacher and Song values share the width and truncate only when necessary. Missing Description, preacher, Song, or Status values consume no space. Vorrade and Notes remain available in the Service editor. Text plus Description form one reliable Text-record tap target, Song remains a separate Song-record tap target, and the rest of the row opens the Service editor. Rows use compact spacing, thin dividers, and subtle press feedback; the toolbar scrolls normally rather than staying fixed. The desktop spreadsheet view is otherwise unchanged.
...
7. Reports page with annual Register preview and printing.
8. Backup/health information for the administrator.
...
- Build create, edit, search, and history views for Songs, Texts, and Vorraden. People are created and reused through the shared service-entry fields. Version 0.20.0 adds the full Vorraden library, identity-safe service editing, protected deletion, merging, and private PDF/photo attachments.
...
- Add multi-PDF and photo upload, list, viewing, download, rename, ordering, and deliberate removal. Version 0.17.0 implements the reusable viewer and photo workflow for Texts; version 0.20.0 applies it to Vorraden; service attachments follow.
...
- Add the exclusive `before:` Last Used command to the existing Texts search in version 0.21.0.
...
3. **Song identity:** the Title, including any number, is the user-facing identity. If multiple hymnals later require otherwise identical titles, add a `hymnals` table and distinguish them in the selector.
4. **Attachment import:** PDF source naming and matching rules remain unknown.
5. **Deletion policy:** unused Texts and Vorraden may be permanently deleted, while referenced records are restricted. Confirm the permanent-deletion and audit rules for Songs and People.
6. **Server platform:** operating system, domain, and TLS method remain deployment choices.
7. **Backup retention:** the number of biweekly and annual Windows copies remains the owner's decision.
8. **Backup encryption:** version 0.14.0 downloads are unencrypted. Decide whether future protection will use Windows Device Encryption/BitLocker, encrypted archives, or destination-level encryption.
9. **Automatic destination:** a second physical machine or NAS at another location is preferred, but the device and transfer method remain open.
